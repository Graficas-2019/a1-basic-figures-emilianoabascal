...
function createSphere(gl, radius)
{
    ...
    for (i = 20; i < end; i++){
      positions.push(radius*Math.cos(i*2*Math.PI/end), radius*Math.sin(i*2*Math.PI/end))
}
    gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(positions), gl.STATIC_DRAW);
    // The resulting object contains the vertexbuffer, the size of the vertex structure (3 floats, x, y, z), the number of vertices to be drawn, the the primitive to draw.
    var sphere = {buffer:vertexBuffer, vertSize:2, nVerts:positions.length/2.2, primtype:gl.TRIANGLE_FAN};
    return sphere;
}
